...
function makeTachometerHUD (L) {
	...
	let loader = new THREE.TextureLoader();
	let alpha = loader.load ('https://i.imgur.com/RXWaQZl.png');
	let base = new THREE.Mesh(new THREE.PlaneGeometry(L, 0.7), new THREE.MeshBasicMaterial({
		alphaMap: alpha,
		color: 'white',
		transparent:true
	}));
	...
	let _bar = new THREE.Group();
	let blood = new THREE.Mesh(new THREE.PlaneGeometry(L, 0.7), new THREE.MeshBasicMaterial({
		color: 'cyan', 
		side:THREE.DoubleSide
	}));
	_bar.add(blood);
...
function person () {
    ...
    let loader = new THREE.TextureLoader();
    let first = loader.load ("https://i.imgur.com/9u1brLw.png");
    let third = loader.load ("https://i.imgur.com/4cplgbb.png");
    
    let firstMesh = new THREE.Mesh (new THREE.CircleGeometry(1,20), new THREE.MeshBasicMaterial(
        {map: first, transparent: true, alphaTest: 1, side:THREE.DoubleSide}));
    let thirdMesh = new THREE.Mesh (new THREE.CircleGeometry(1,20), new THREE.MeshBasicMaterial(
        {map: third, transparent: true, alphaTest: 1, side:THREE.DoubleSide}));
	let personstateSign = new THREE.Group();
    personstateSign.add (firstMesh,thirdMesh);
    return personstateSign;
